...
{
    public CreateEventDtoValidator(IEventRepository eventRepository)
    {
        RuleFor(r => r.Name)
            .NotEmpty()
            ...
            .MustAsync(async (eventName, cancellation) =>
            {
                bool exists = await eventRepository.IsEventNameTaken(eventName, cancellationToken: cancellation);
                return !exists;
            })
            .WithMessage("Event name is already taken");
